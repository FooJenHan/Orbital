...
class ViewTests(TestCase):
    ...
    def test_mainpage_rendered(self):
        url = reverse("index")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'mainpage/index.html')

    def test_modlist_rendered(self):
        url = reverse("mod_list")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'mainpage/mod_list.html')

    def test_gradplan_rendered(self):
        url = reverse("grad_plan")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'mainpage/grad_plan.html')
